...
class Transformer(ModuleBase):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
        ...
        self.row_embed = torch.nn.Embedding(35, 192 // 2)
        self.col_embed = torch.nn.Embedding(35, 192 // 2)
        self.avgpol = nn.AdaptiveAvgPool2d(1)
        self.sig = nn.Sigmoid()

    def forward(self, img_pos, img_neg, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
                memory_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        all_input = self.downsampler1(self.concat(torch.cat([img_pos[0], img_pos[1], img_pos[2], img_pos[3], img_pos[4], \
